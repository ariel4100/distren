    ...
    <ul class="list-unstyled">
        @forelse($categorias as $key=>$item)
            <li class="list-group-item border-0 px-0">
                <a href="{{ route('categoria',$item->id) }}" data-target="#categoria_{{$key}}" data-toggle="collapse" aria-expanded="false" class="d-flex align-items-center p-2 border-bottom {{ $item->id == $categoria->product[0]->category_id ? 'distren-color' : null }}">
                    <span onclick="location.href='{{ route('categoria',$item->id) }}'">{!! $item->title !!}</span><i class="fas fa-chevron-right ml-auto"></i>
                </a>
                <ul class="list-unstyled collapse {{ $item->id == $categoria->product[0]->category_id ? 'show' : null }}" id="categoria_{{$key}}">
